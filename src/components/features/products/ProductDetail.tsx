...
const ProductDetail = ({ product }: { product: any }) => {
  ...
  const [selectedEggOption, setSelectedEggOption] = useState<any>(
    product.eggOptions && product.eggOptions.length > 0 ? product.eggOptions[0] : null
  );
  ...
  const effectivePrice = hasVariants && selectedSize
    ? selectedSize.price + (selectedEggOption ? selectedEggOption.priceAdd : 0)
    : product.price;
  ...
  const handleIncrease = () => {
    if (cartItem) {
      updateQuantity(product.id, cartItem.quantity + 1, variantKey);
    } else {
      addToCart(
        { id: product.id, name: product.name, price: effectivePrice, quantity: 1, image: product.image, variantKey, variantLabel },
        { silent: true }
      );
    }
  };
  ...
  const handleDecrease = () => {
    if (!cartItem) return;
    if (cartItem.quantity <= 1) removeFromCart(product.id, variantKey);
    else updateQuantity(product.id, cartItem.quantity - 1, variantKey);
  };
  ...
  const handleAddToBag = () => {
    ...
    if (customMessage.trim()) {
      updateItemNote(product.id, customMessage.trim(), variantKey);
    }
  ...
  const handleWhatsAppOrder = () => {
    ...
    const msg = encodeURIComponent(
      `Hi! I'd like to order:\n*${displayQty}x ${product.name}* (KES ${(effectivePrice * displayQty).toLocaleString()})${variantLine}${noteLine}\n\nIs this available?`
    );
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  <VariantDropdown
                    ...
                    renderOption={(option) => option.label + " — KES " + option.price.toLocaleString()}
                  />
                </div>

                {product.eggOptions && product.eggOptions.length > 0 && (
                  <div className="space-y-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted ml-1">Egg Option</span>
                    <VariantDropdown
                      options={product.eggOptions}
                      value={selectedEggOption?.label || ""}
                      onChange={(option) => setSelectedEggOption(option)}
                      renderOption={(option) =>
                        option.label + (option.priceAdd > 0 ? " (+KES " + option.priceAdd.toLocaleString() + ")" : "")
                      }
                    />
                  </div>
                )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] text-muted font-bold">
                  Total: <span className="text-foreground font-black">KES {(effectivePrice * displayQty).toLocaleString()}</span>
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-muted uppercase tracking-widest">
                  {displayQty} × KES {effectivePrice.toLocaleString()}
                </span>
                <span className="font-black text-sm text-foreground italic">
                  KES {(effectivePrice * displayQty).toLocaleString()}
                </span>
              </div>
            </div>
            <Button
